...
In general, the computation of the simple Nystr\"om extension is unstable because it requires one to take the pseudoinverse of a matrix $\mat{W}$ which may be nearly singular. Unless it is known \textit{a priori} that the minimum eigenvalue of $\mat{W}$ is bounded away from zero, one should instead compute an approximation to the Nystr\"om extension using a stable algorithm. We propose two such algorithms.
...
\begin{algorithm}
 ...
 \begin{algorithmic}[1]
  \Require{ $\mat{A}$, an $n \times n$ SPSD matrix; an integer $k \geq \rank(\mat{A});$ a regularization parameter $\rho > 0;$ and an integer $\ell > k.$ }
...
Algorithm~\ref{alg:stable-nystrom} returns a Nystr\"om extension where the matrix $\mat{C}$ is taken from $\mat{A},$ but the coupling matrix $\mat{W}_\rho$ is taken from $\mat{A}_\rho.$ Intuitively, for small $\rho,$ the approximation returned by Algorithm~\ref{alg:stable-nystrom}, $\mat{C}\mat{W}_\rho^{-1} \mat{C}^\transp,$ should be a better approximation to $\mat{A}$ that that returned by Algorithm~\ref{alg:simple-stable-nystrom}, $\mat{C}_\rho \mat{W}_\rho^{-1} \mat{C}_\rho^\transp,$ because it uses more information from $\mat{A}.$ However, it is not clear that this intuition should hold for larger $\rho$: the added accuracy from using $\mat{C}$ may be negated by the fact that $\mat{C}$ and $\mat{W}_\rho$ are from two different matrices, so $\mat{C}\mat{W}_\rho^{-1} \mat{C}^\transp$ can no longer be thought of as a Nystr\"om extension of a matrix close to $\mat{A},$ whereas $\mat{C}_\rho \mat{W}_\rho^{-1} \mat{C}_\rho^\transp$ is always a Nystr\"om extension of a matrix that is close to $\mat{A}.$
...
\begin{algorithm}
 ...
 \algrenewcommand\algorithmicensure{\textbf{Output:}}
 \begin{algorithmic}[1]
  \Require{ $n \times n$ SPSD matrix $\mat{A}$, an integer $k > \rank(\mat{A}),$ a tolerance $\rho > 0,$ and an integer $\ell > k.$ }
...
\begin{figure}[ht!]
 ...
 % optimalityverification.pdf: 393x309 pixel, 72dpi, 13.86x10.90 cm, bb=0 0 393 309
...
\centering
 \subfloat[][Relative spectral norm errors of Algorithm~\ref{alg:simple-stable-nystrom}]{%
  \includegraphics[width=6in,keepaspectratio=true]{./experiments/coherence-regularizedA.pdf}
 % optimalityverification.pdf: 393x309 pixel, 72dpi, 13.86x10.90 cm, bb=0 0 393 309
}

 %\label{fig:coherence-regularizedA}
\subfloat[][Relative spectral norm errors of Algorithm~\ref{alg:stable-nystrom}]{% 
 \includegraphics[width=6in,keepaspectratio=true]{./experiments/coherence-regularizedW.pdf}
 % optimalityverification.pdf: 393x309 pixel, 72dpi, 13.86x10.90 cm, bb=0 0 393 309
}

\subfloat[][Relative spectral norm errors of the algorithm of~\cite{CD11}]{%
 \includegraphics[width=6in,keepaspectratio=true]{./experiments/coherence-truncatedw.pdf}
 % optimalityverification.pdf: 393x309 pixel, 72dpi, 13.86x10.90 cm, bb=0 0 393 309
}
